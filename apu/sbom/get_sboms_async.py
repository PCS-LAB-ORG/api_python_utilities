...
for dep in dependencies:
    ...
    for cve in dep['cves']:
        ...
        try:
            ...
            pass
        try:
            cve['publishedDate'] = dt.strptime(str(published_date), "%Y-%m-%dT%H:%M:%SZ")
            continue
...
for index, dependency in enumerate(dependencies):
    logger.info(f"{dependency['srcCount']} repos ({index}/{dep_count} package-versions) {dependency['id']}")
...
while should_retry and retry_count > 0 and len(retry_sources) > 0:
    ...
    for key, package in repos_found.items():
        ...
        sorted_cves = sorted(dependency['cves'], key=lambda cve: cve["publishedDate"], reverse=True)
        cve_id = ""
