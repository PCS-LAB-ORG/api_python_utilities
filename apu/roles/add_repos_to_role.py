    ...
    if not current_role_list:
        ...
        with open(f".role_backup-{now}", "w+", newline="") as role_backup_file:
            writer = csv.DictWriter(
                role_backup_file, fieldnames=current_role_list[0].keys()
            )
            writer.writeheader()
            writer.writerows(current_role_list)
        print(
            f"Took a backup of the roles before making changes and saved to {role_backup_file_name}. \
    ...
    for role in old_role_list_file_content:
        ...
        if not 0 == role["codeRepositoryIds"].count(","):
            for repo_id in (
                ...
                try:
                    new_repo_id_list.append(
                        old_repo_id_map[repo_id]
                    )  # use the old ID to get the URL
                except KeyError as e:
                    print(e)
        role_url_mapper[role["id"]] = (
            new_repo_id_list  # update the role with the new ID list
        )
